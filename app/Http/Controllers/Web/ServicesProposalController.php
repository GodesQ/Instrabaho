...
class ServicesProposalController extends Controller
{
    public function submit_proposal(Request $request) {
        $request->validate([
            ...
            'estimated_finish_date' => 'required',
            'message' => 'required',
            'location' => 'required',
            'latitude' => 'required',
            'longitude' => 'required',
    ...
    public function employer_proposals(Request $request) {
        $user_id = session()->get('id');
        $role = session()->get('role');
        $user = Employer::where('user_id', $user_id)->first();
        $purchased_services = ServicesProposal::where('buyer_id', $user->id)
        ->where('isCancel', 0)
