...
class HomeScreenController extends Controller
{
    ...
    {
        $freelancers = Freelancer::limit(8)
            ->with('user', 'skills')
            ->get();
        $projects = Project::where('status', 'pending')
            ...
            ->latest('id')
            ->get();
        $user = session()->get('role') == 'freelancer' ? Freelancer::where('user_id', session()->get('id'))->first() : Employer::where('user_id', session()->get('id'))->first();
        return view('welcome', compact('freelancers', 'projects', 'user'));
    }
    ...
    public function freelancer(Request $request)
    {
        ...
        $user = User::where('username', $request->username)
            ->with('freelancer')
            ->firstOrFail();
        $freelancer = Freelancer::where('user_id', $user->id)
            ->with('user', 'certificates', 'experiences', 'educations', 'services', 'skills', 'projects', 'projects_completed')
            ->firstOrFail();
        $active_services = $freelancer
            ->services()
            ->where('expiration_date', '>', Carbon::now())
            ->get();
        $featured_services = $freelancer
            ->services()
            ->where('type', 'featured')
            ->where('expiration_date', '>', Carbon::now())
            ->get();

        $my_profile = Employer::where('user_id', session()->get('id'))
            ->with('projects')
            ->first();

        $reviews = FreelancerReview::where('freelancer_id', $freelancer->id)
        ...
        if ($my_profile) {
            $follow_freelancer = FreelancerFollower::where('freelancer_id', $freelancer->id)
                ->where('follower_id', $my_profile->id)
                ->exists();
        }
    ...
    {
        $employer = Employer::where('user_id', $request->id)
            ->with('user', 'projects')
            ->firstOrFail();
        $completed_projects = Project::where('employer_id', $employer->id)
            ->where('status', 'completed')
            ->get();

        $my_profile = Freelancer::where('user_id', session()->get('id'))->first();
        $follow_employer = $my_profile
            ? ($follow_employer = EmployerFollower::where('employer_id', $employer->id)
                ->where('follower_id', $my_profile->id)
                ->exists())
            : false;
        ...
        $services = Service::select('*')
            ...
            * cos(radians(services.longitude) - radians(" .
            ...
            + sin(radians(" .
                                $latitude .
                                "))
            * sin(radians(services.latitude))) AS distance",
                        ),
                    )
                    ->having('distance', '<=', '10')
    ...
    public function service(Request $request)
    {
        $service = Service::where('id', $request->id)
            ->with('category', 'freelancer')
            ->firstOrFail();
        $addons = Addon::where('user_role_id', $service->freelancer_id)
            ->limit(5)
            ->get();
        ...
        $projects = Project::select('*')
            ...
            * cos(radians(projects.longitude) - radians(" .
            ...
            + sin(radians(" .
                                $user->latitude .
                                "))
            * sin(radians(projects.latitude))) AS distance",
                        ),
                    )
                    ->having('distance', '<=', '10')
                    ->orderBy('distance', 'asc')
    ...
    public function project(Request $request)
    {
        ...
        $save_project = $freelancer
            ? SaveProject::where('project_id', $project->id)
                ->where('follower_id', $freelancer->id)
                ->first()
            : null;
        $proposal = $freelancer
            ? ProjectProposal::where('project_id', $project->id)
    ...
    public function fetch_freelancers(Request $request)
    {
        ...
        $freelancers = Freelancer::select('*')
            ->when($title, function ($q) use ($title) {
                return $q
                    ->where(DB::raw('lower(display_name)'), 'like', '%' . strtolower($title) . '%')
            ...
            * cos(radians(user_freelancer.longitude) - radians(" .
                                $longitude .
                                "))
            + sin(radians(" .
                                $latitude .
                                "))
            * sin(radians(user_freelancer.latitude))) AS distance",
                        ),
                    )
                    ->having('distance', '<=', $radius)
                    ->orderBy('distance', 'asc');
