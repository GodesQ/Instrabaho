...
@section('content')
    ...
    <div class="container-fluid my-2">
        <h2 class="font-weight-bold">Welcome <span class="primary h2 font-weight-bold">{{ $freelancer->display_name }}</span>!
        </h2>
                        ...
                        @forelse ($projects as $project)
                            <div class="col-xxl-4 col-xl-6">
                                <div class="card rounded"
                                    style="box-shadow: 5px 5px 5px 0px rgba(181, 230, 250, 0.22) !important;">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div>
                                                <a href="/projects/{{ $project->id }}/{{ $project->title }}"
                                                    class="font-weight-medium h4 info">{{ strlen($project->title) > 15 ? substr($project->title, 0, 15) . '...' : $project->title }}</a>
                                                <div class="warning">
                                                    {{ $project->employer->user->firstname . ' ' . $project->employer->user->lastname }}
                                                </div>
                                                <div class="font-weight-bold">Location: <span
                                                        class="font-weight-normal">{{ substr($project->location, 0, 20) }}...</span>
                                                </div>
                                                <div class="font-weight-bold">Distance: <span
                                                        class="font-weight-normal">{{ number_format($project->distance, 2) }}
                                                        km</span></div>
                                            </div>
                                        </div>
                                        <div class=" my-1 d-flex justify-content-end align-items-center">
                                            <a href="/projects/{{ $project->id }}/{{ $project->title }}"
                                                class="btn btn-outline-primary mr-50">View Project <i
                                                    class="fa fa-eye"></i></a>
                                            <a href="/projects/{{ $project->id }}/{{ $project->title }}#fr-bid-form"
                                                class="btn btn-primary">Apply <i class="fa fa-send"></i></a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @forelse ($recent_projects as $recent_project)
                                    <tr>
                                        <td>
                                            <a href="/projects/{{ $recent_project->project->id }}/{{ $recent_project->project->title }}"
                                        ...
                                        <td class="text-right">
                                            <span class="dropdown">
                                                <a id="btnSearchDrop7" data-toggle="dropdown" aria-haspopup="true"
                                                    aria-expanded="true" class="dropdown-toggle dropdown-menu-right"><i
                                                        class="fa fa-ellipsis-v"></i></a>
                                                <span aria-labelledby="btnSearchDrop7"
                                                    class="dropdown-menu mt-1 dropdown-menu-right">
                                                    <a href="/projects/{{ $recent_project->project->id }}/{{ $recent_project->project->title }}"
                                                        class="dropdown-item delete"><i class="feather icon-eye"></i> View
                                                        Project</a>
                                                    <a href="/project/contract/view/{{ $recent_project->id }}"
                                                        class="dropdown-item"><i class="feather icon-file "></i> View
                                                        Contract</a>
                                                    <a href="/project/contract/track/{{ $recent_project->id }}"
                                                        class="dropdown-item"><i class="feather icon-file "></i> View Track
                                                        Record</a>
                                                </span>
                                            </span>
                                        </td>
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @forelse ($projects_schedule_week as $project)
                                <tr>
                                    <td>
                                        <a href="/projects/{{ $project->project->id }}/{{ $project->project->title }}"
                                            class="font-weight-medium primary">
                                        ...
                                        <span class="dropdown">
                                            ...
                                            <span aria-labelledby="btnSearchDrop7"
                                                ...
                                                <a href="" class="dropdown-item delete"><i
                                                        class="feather icon-x"></i> Cancel Project</a>
                                                <a href="/projects/{{ $project->project->id }}/{{ $project->project->title }}"
                                                    class="dropdown-item delete"><i class="feather icon-eye"></i> View
                                                    Project</a>
                                                <a href="/project/contract/view/{{ $project->id }}"
                                                    class="dropdown-item"><i class="feather icon-file "></i> View
                                                    Contract</a>
                                            </span>
                                        </span>
