                        ...
                        <div class="col-xl-4 col-lg-6">
                            <div class="card">
                                <div class="card-body">
                                            ...
                                            <span aria-labelledby="btnSearchDrop7"
                                                ...
                                                @if ($data->contract)
                                                    <a href="/project/contract/track/{{ $data->contract->id }}"
                                                        class="dropdown-item "><i class="feather icon-activity primary"></i>
                                                        Start Working</a>
                                                @endif
                                    ...
                                    <p>{{ strlen($data->project->description) > 150 ? substr($data->project->description, 0, 150) . '...' : $data->project->description }}
                                    </p>
                                    <ul>
                                        <li><i class="fa fa-calendar mr-1"></i> Start Date : <span
                                                class="font-weight-bold">{{ $data->contract && $data->contract->start_date ? date_format(new DateTime($data->contract->start_date), 'F d, Y') : 'No Start Date' }}</span>
                                        </li>
                                        <li><i class="fa fa-calendar mr-1"></i> End Date : <span
                                                class="font-weight-bold">{{ $data->contract && $data->contract->end_date ? date_format(new DateTime($data->contract->end_date), 'F d, Y') : 'No End Date' }}</span>
                                        </li>
                                    </ul>
                                    <div class="text-right">
                                        @if ($data->contract)
                                            <a href="/project/contract/view/{{ $data->contract ? $data->contract->id : 0 }}"
