                ...
                <div class="d-flex justify-content-center my-2">
                    <div class="container" style="width: 90% !important;">
                        <div class="d-flex p-2" style="gap: 20px; box-shadow: 1px 0px 20px rgb(0 0 0 / 7%);">
                            <div>
                                <img class="rounded"
                                    src="../../../images/user/profile/{{ $pending_offer->employer->user->profile_image }}"
                                    alt="employer_image" style="height: 100px !important;">
                            </div>
                            <div>
                                <div style="color: #003066;">{{ $pending_offer->employer->user->firstname }}
                                    {{ $pending_offer->employer->user->lastname }}</div>
                                <h3 class="text-secondary">{{ $pending_offer->employer->display_name }}</h3>
                                <p class="font-weight-light">
                                    {{ substr($pending_offer->employer->description, 0, 255) }}...</p>
                                <div class="text-right mt-2">
                                    <a href="/employer/{{ $pending_offer->employer->user_id }}"
                                        class="btn btn-outline-primary">View Client <i class="fa fa-user"></i></a>
                                    <a href="/service_proposal_information/{{ $pending_offer->id }}"
                                        class="btn btn-success">Approved Offer <i class="fa fa-thumbs-up"></i></a>
                                    <a href="/service_proposal_information/{{ $pending_offer->id }}"
                                        class="btn btn-primary">View Offer <i class="fa fa-arrow-right"></i></a>
                                </div>
                            </div>
                        </div>
                    </div>
